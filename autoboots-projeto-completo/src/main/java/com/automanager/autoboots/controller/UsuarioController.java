...
public class UsuarioController {
    ...
    public ResponseEntity<CollectionModel<Usuario>> listarTodos() {
        ...
        if (role.equals("ROLE_CLIENTE")) {
            ...
            // VENDEDOR vê CLIENTES + VENDEDORES
            usuarios = usuarios.stream()
                    .filter(u -> u.getRole().equals("CLIENTE") || u.getRole().equals("VENDEDOR"))
                    .collect(Collectors.toList());
        } else if (role.equals("ROLE_GERENTE")) {
            // GERENTE vê CLIENTES + VENDEDORES + GERENTES (todos exceto ADMIN)
            usuarios = usuarios.stream()
                    .filter(u -> !u.getRole().equals("ADMINISTRADOR"))
                    .collect(Collectors.toList());
        }
        // ADMINISTRADOR vê todos (sem filtro)
                    ...
                    if (role.equals("ROLE_CLIENTE")) {
                        ...
                        if (!usuario.getUsername().equals(username)) {
                            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                                    .body("CLIENTE só pode visualizar próprio cadastro");
                        }
                    } else if (role.equals("ROLE_VENDEDOR")) {
                        // VENDEDOR só pode ver CLIENTES e VENDEDORES
                        if (!usuario.getRole().equals("CLIENTE") && !usuario.getRole().equals("VENDEDOR")) {
                            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                                    .body("VENDEDOR só pode visualizar usuários do tipo CLIENTE e VENDEDOR");
                        }
                    } else if (role.equals("ROLE_GERENTE")) {
                        // GERENTE pode ver todos exceto ADMINISTRADOR
                        if (usuario.getRole().equals("ADMINISTRADOR")) {
                            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                                    .body("GERENTE não pode visualizar usuários do tipo ADMINISTRADOR");
                        }
                    }
        ...
        return usuarioRepository.findById(id)
                .map(usuario -> {
                    if (role.equals("ROLE_VENDEDOR")) {
                        if (!usuario.getRole().equals("CLIENTE")) {
                            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                                    .body("VENDEDOR só pode atualizar usuários do tipo CLIENTE");
                        }
                    } else if (role.equals("ROLE_GERENTE")) {
                        if (usuario.getRole().equals("ADMINISTRADOR")) {
                            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                                    .body("GERENTE não pode atualizar usuários do tipo ADMINISTRADOR");
                        }
                        if (usuarioAtualizado.getRole().equals("ADMINISTRADOR")) {
                            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                                    .body("GERENTE não pode promover usuários para ADMINISTRADOR");
                        }
                    }
